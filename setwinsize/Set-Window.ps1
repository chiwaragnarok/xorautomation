...
.PARAMETER TitleName
Name of the title to determine the window characteristics. 
...
Process {
    ...
    If ( $PSBoundParameters.ContainsKey('Id') ) {
        ...
        $Processes = Get-Process -ErrorAction SilentlyContinue | Where-Object { $_.MainWindowTitle -eq $TitleName }
		If ( $PSBoundParameters['ToFront'] ) {
			(New-Object -ComObject WScript.Shell).AppActivate($TitleName) | Out-Null
            Start-Sleep 0.2
		}
    }
    if ( $null -eq $Processes ) {
        ...
        $Processes | ForEach-Object {
            ...
            If ($Rectangle.Top    -ge 0 -OR 
                $Rectangle.Bottom -ge 0 -OR
                $Rectangle.Left   -ge 0 -OR
                $Rectangle.Right  -ge 0) {
            ...
            If ( $Return ) {
                #[Window]::ShowWindow($Handle, 3) | Out-Null
                #Start-Sleep 0.5
                $Return = [Window]::MoveWindow($Handle, $x, $y, $Width, $Height,$True)
                #Start-Sleep 0.5
                #[Window]::ShowWindow($Handle, 10) | Out-Null
            }
            If ( $PSBoundParameters['Passthru'] ) {
                $Rectangle = New-Object RECT
                $Return = [Window]::GetWindowRect($Handle,[ref]$Rectangle)
